...
import ReadFinancial from "../src";
import fs from "fs";

describe("Reader testing", () => {
  for (const filePath of fs.readdirSync(
    path.resolve("__test__/financialReport")
  )) {
    const financeInsEn = new ReadFinancial({
      path: path.resolve(`__test__/financialReport/${filePath}`),
      language: "en",
    });
    ...
    it(`General data of ${fileName}`, async () => {
      const [enGeneral, idGeneral] = [
        await financeInsEn.getGeneral(),
        await financeInsId.getGeneral(),
      ];
    ...
    it(`Balance data of ${fileName}`, async () => {
      const [enBalance, idBalance] = [
        await financeInsEn.getBalance(),
        await financeInsId.getBalance(),
      ];
    ...
    it(`Income data of ${fileName}`, async () => {
      const [enIncome, idIncome] = [
        await financeInsEn.getIncome(),
        await financeInsId.getIncome(),
      ];
    ...
    it(`Cashflow data of ${fileName}`, async () => {
      const [enCashflow, idCashflow] = [
        await financeInsEn.getCashflow(),
        await financeInsId.getCashflow(),
      ];
    ...
    it(`Change Statement data of ${fileName}`, async () => {
      const [enGeneral, idGeneral] = [
        await financeInsEn.getChangeStatement(),
        await financeInsId.getChangeStatement(),
      ];
      const idKeyLength = Object.keys(idGeneral).length;
